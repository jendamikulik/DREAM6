        ...
        #try:
            ...
            for branch_name, th_test in candidates:
                ...
                assign_test, score_test = extract_assignment_from_ipc(
                    clauses, nvars,
                    clause_phasors=a_eff, theta=th_test, clause_weights=w
                )

                unsat_test = count_unsat(clauses, assign_test)

                op_diag = operator_diagnostics(a_eff, th_test, w)
                soft_diag = soft_reading_diagnostics(
                    a_eff, th_test, w,
                    epsilon=max(1e-12, float(closure_epsilon)),
                )
            ...
            if polish > 0:
                assign = greedy_polish(clauses, model_assign, flips=polish, seed=seed, report_every=report_every)
...
def greedy_polish(
    ...
    probsat_quota: int = 20_000,  # max kroků pro probSAT fázi v rámci polish,
    ...
    print(f'\n[POLISH] steps:{steps} report_every:{report_every}')
    ...
    while steps < flips:
        ...
        if report_every > 0 and (steps % report_every == 0):
            cur_uns = cur_unsat()
            print(f"[POLISH] step={steps}  unsat={cur_uns}/{C}  ({100.0 * cur_uns / max(1, C):.4f}%)")
        ...
        if steps % 5000 == 0 and u >= best_uns:
...
def greedy_polish(
    ...
    beta: float = 0.9,      # probSAT break exponent
    epsilon: float = 1e-3,  # probSAT epsilon
    probsat_quota: int = 20000,   # max kroků pro probSAT burst
    ...
    """
    Micro-polish finisher:
      A) globálně vyčerpá all zero-break flips (tie -> max-make)
      B) min-break + max-make + tabu
      C) probSAT burst při stagnaci
      D) exact_core_repair při 1..4 UNSAT

    Vstup/výstup: assign01 je 0/1 list délky nvars.
    """
    rnd = random.Random(seed)
    nvars = len(assign01)
    C = len(clauses)
    stall_trigger = 5000  # po tolika krocích bez zlepšení pustíme burst
    ...
    # živý UNSAT container: O(1) add/drop/pick/count
    unsat: List[int] = []
    unsat_pos = [-1] * C
    ...
    def breakcount(v: int) -> int:
        bc = 0
        if assign[v]:  # True -> False, zhasínáme pozitivní literály
            for ci in pos[v]:
                if sat_count[ci] == 1:
                    bc += 1
        else:          # False -> True, zhasínáme negativní literály
            for ci in neg[v]:
                if sat_count[ci] == 1:
                    bc += 1
        return bc
    ...
    def makecount(v: int) -> int:
        mk = 0
        if assign[v]:  # True -> False, rozsvítíme negativní literály
            for ci in neg[v]:
                if unsat_pos[ci] != -1:
                    mk += 1
        else:          # False -> True, rozsvítíme pozitivní literály
            for ci in pos[v]:
                if unsat_pos[ci] != -1:
                    mk += 1
        return mk
    ...
    def flip_var(v: int) -> None:
        ...
        if old:
            # pozitivní literály ztrácejí truth
            for ci in pos[v]:
                sc = sat_count[ci] - 1
                sat_count[ci] = sc
                if sc == 0:
                    add_unsat(ci)
            # negativní literály gain truth
            for ci in neg[v]:
                sc = sat_count[ci] + 1
                sat_count[ci] = sc
                if sc == 1:
            ...
            # negativní literály ztrácejí truth
            for ci in neg[v]:
                sc = sat_count[ci] - 1
                sat_count[ci] = sc
                if sc == 0:
                    add_unsat(ci)
            # pozitivní literály gain truth
            for ci in pos[v]:
                sc = sat_count[ci] + 1
                sat_count[ci] = sc
                if sc == 1:
    ...
    def choose_var_from_clause(clause: List[int], steps: int) -> Optional[int]:
        best_free = None       # (make, v)
        best_cand = None       # (break, -make, v)

        best_free_any = None   # fallback bez tabu
        best_cand_any = None   # fallback bez tabu
        ...
        for L in clause:
            ...
            # fallback kandidáti bez ohledu na tabu
            if bc == 0:
                item_free_any = (mk, v)
            ...
            # normální výběr respektuje tabu
        ...
        # když je všechno tabu, povol fallback
    ...
    # -------- Phase A: globálně vyčerpej freebies --------
    while steps < flips and unsat:
        made_progress = False

        # scan přes snapshot aktuálních UNSAT klauzulí;
        # po každém freebie flipu scan restartujeme
    ...
    while steps < flips and unsat:
        ...
        # -------- Phase C: probSAT burst při stagnaci --------
    ...
    # budget vyčerpán nebo degenerace -> vrať nejlepší nalezené
    return [1 if b else 0 for b in best_assign[1:]]
