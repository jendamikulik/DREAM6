#!/usr/bin/env python3
"""
DREAM6 v187 -- experimental universal relation calculus over the certified v186 core.

Research objective
------------------
Replace benchmark/family semantic recognizers by a recursive calculus on exact
finite Boolean relations.  v187 adds a family-blind audit layer that derives:

* repeated-support exact relations by local truth-state enumeration,
* exact and unique-maximal affine hyperplane shadows,
* fixed Hamming-layer support closures,
* strong multi-variable overlap components,
* compact low-arity interaction components.

The first old named chart actually removed from the solving path is the
trimmed-parity/two-trees case: v187 derives unique maximal affine subrelations
from arbitrary local relations, forms the global GF(2) system, and performs
exact symbolic closure against the untouched CNF.

For structures not yet replaced end-to-end, v187 keeps the certified v186
solver as an explicit conservative bridge.  The audit and the bridge never
read benchmark filenames or family labels.  There is no residual-guided
repair, variable flipping, WalkSAT, branching, restart portfolio, random
noise, or external SAT solver.  SAT is emitted only after independent exact
verification of the original CNF.

Current regression checkpoint (seed 0, supplied benchmark set)
---------------------------------------------------------------
UF250                         1065 / 1065 SAT
ac-sgen                        432 / 432 SAT
two-trees                    2039 / 2039 SAT  [native v187 affine-shadow path]
MDP                          4438 / 4438 SAT
X9                           5406 / 5406 SAT
Battleship                   2562 / 2562 SAT
RB-SAT                      84314 / 84314 SAT
FRB                         24731 / 24731 SAT
APN S-box                   86081 / 86081 SAT
PCMax scheduling            13561 / 13561 SAT

This is an experimental research version, not a proof of SAT completeness and
not a proof of P=NP.
"""
...
VERSION = "v187_EXPERIMENTAL_UNIVERSAL_RELATION_CALCULUS_PLUS_V186_CERTIFIED_FALLBACK"
...
def _v186_emit_section(
    ...
    *,
    ...
    sat = unsat == 0
    stem = Path(args.cnf_path).stem
    model_path = Path(args.model_out or (f"{stem}_v187.model" if sat else f"{stem}_v187.candidate.model"))
    residual_path = model_path.with_suffix(".unsat.txt")
    write_model(model_path, assignment, sat)
    write_residual(residual_path, cnf, residual_ids)
...
# ---------------------------------------------------------------------------
# v187: experimental universal relation calculus
# ---------------------------------------------------------------------------
#
# This front-end deliberately does not read benchmark/family names.  It
# derives small exact Boolean relations directly from clause supports and
# records representation-independent invariants before any frozen v186 path
# is selected.  The v186 certified paths remain as a conservative fallback
# while their family-specific recognizers are progressively replaced.
#
# The first constructive replacement implemented here is the generic affine-
# shadow selector: any repeated small-support relation having a unique maximal
# codimension-one affine subrelation contributes one formula-derived GF(2)
# equation.  If the global affine system leaves only a small exact free space,
# every remaining section is checked symbolically against the untouched CNF.
#

@dataclass(frozen=True)
class V187RelationAtom:
    support: tuple[int, ...]             # zero-based Boolean variables
    clause_ids: tuple[int, ...]
    state_count: int
    hamming_weights: tuple[int, ...]
    exact_affine_hyperplane: tuple[int, int] | None
    unique_max_affine_hyperplane: tuple[int, int] | None


@dataclass(frozen=True)
class V187UniversalReport:
    repeated_support_atoms: int
    exact_affine_atoms: int
    unique_affine_shadow_atoms: int
    affine_shadow_rank: int
    affine_shadow_free: int
    hamming_layer_atoms: int
    strong_overlap_components: tuple[int, ...]
    low_arity_components: tuple[tuple[int, int, int], ...]
    wide_support_closures: int
    notes: tuple[str, ...]


def _v187_clause_truth(clause: tuple[int, ...], values: dict[int, int]) -> bool:
    for literal in clause:
        variable = abs(int(literal)) - 1
        bit = int(values[variable])
        if (literal > 0 and bit == 1) or (literal < 0 and bit == 0):
            return True
    return False


def _v187_relation_states(cnf: CNF, clause_ids: tuple[int, ...], support: tuple[int, ...]) -> tuple[int, ...]:
    k = len(support)
    states: list[int] = []
    for word in range(1 << k):
        values = {variable: ((word >> j) & 1) for j, variable in enumerate(support)}
        if all(_v187_clause_truth(cnf.clauses[ci], values) for ci in clause_ids):
            states.append(word)
    return tuple(states)


def _v187_affine_hyperplanes(states: tuple[int, ...], k: int, *, contained: bool) -> list[tuple[int, int]]:
    relation = set(int(x) for x in states)
    wanted = 1 << max(0, k - 1)
    out: list[tuple[int, int]] = []
    for mask in range(1, 1 << k):
        for rhs in (0, 1):
            hyper = {x for x in range(1 << k) if (((mask & x).bit_count() & 1) == rhs)}
            if contained:
                if len(hyper) == wanted and hyper.issubset(relation):
                    out.append((mask, rhs))
            elif hyper == relation:
                out.append((mask, rhs))
    return out
...
def _v187_same_support_atoms(cnf: CNF, max_support: int = 8) -> tuple[V187RelationAtom, ...]:
    groups: dict[tuple[int, ...], list[int]] = defaultdict(list)
    for ci, clause in enumerate(cnf.clauses):
        support = tuple(sorted({abs(int(literal)) - 1 for literal in clause}))
        if 1 <= len(support) <= max_support:
            groups[support].append(ci)
    atoms: list[V187RelationAtom] = []
    for support, ids0 in sorted(groups.items(), key=lambda item: (len(item[0]), item[0])):
        # A duplicate identical clause carries no new local semantics.  Require
        # at least two distinct clauses before treating a repeated support as
        # a nontrivial semantic atom.
        distinct = {tuple(sorted(cnf.clauses[i])) for i in ids0}
        if len(distinct) < 2:
            continue
        ids = tuple(ids0)
        states = _v187_relation_states(cnf, ids, support)
        k = len(support)
        exact = _v187_affine_hyperplanes(states, k, contained=False)
        contained = _v187_affine_hyperplanes(states, k, contained=True)
        weights = tuple(sorted({int(x).bit_count() for x in states}))
        atoms.append(V187RelationAtom(
            support=support,
            clause_ids=ids,
            state_count=len(states),
            hamming_weights=weights,
            exact_affine_hyperplane=exact[0] if len(exact) == 1 else None,
            unique_max_affine_hyperplane=contained[0] if len(contained) == 1 else None,
        ))
    return tuple(atoms)


def _v187_gf2_rank_rows(rows: list[int]) -> int:
    basis: dict[int, int] = {}
    for value0 in rows:
        value = int(value0)
        while value:
...
def _v187_affine_shadow_equations(atoms: tuple[V187RelationAtom, ...]) -> tuple[list[int], list[int]]:
    rows: list[int] = []
    rhs: list[int] = []
    for atom in atoms:
        local = atom.unique_max_affine_hyperplane
        if local is None:
            continue
        local_mask, bit = local
        global_mask = 0
        for j, variable in enumerate(atom.support):
            if (local_mask >> j) & 1:
                global_mask |= 1 << int(variable)
        rows.append(global_mask)
        rhs.append(int(bit))
    return rows, rhs


def _v187_low_arity_components(cnf: CNF, width: int = 2, max_component: int = 16):
    adjacency: dict[int, set[int]] = defaultdict(set)
    low_ids: list[int] = []
    for ci, clause in enumerate(cnf.clauses):
        support = tuple(sorted({abs(int(x)) - 1 for x in clause}))
    ...
    for start in sorted(adjacency):
        ...
        if len(comp) > max_component:
            continue
        ids = tuple(
            ci for ci in low_ids
            if {abs(int(x)) - 1 for x in cnf.clauses[ci]}.issubset(comp)
        )
        support = tuple(sorted(comp))
        states = _v187_relation_states(cnf, ids, support)
        summary.append((len(comp), len(ids), len(states)))
    return tuple(sorted(summary))


def _v187_wide_hamming_closures(cnf: CNF, min_width: int = 4, max_width: int = 8):
    supports = [frozenset(abs(int(x)) - 1 for x in clause) for clause in cnf.clauses]
    # Inverted variable -> clauses index makes support closure inexpensive on
    # the present regression suite and avoids a benchmark-family catalogue.
    by_var: dict[int, set[int]] = defaultdict(set)
    for ci, support in enumerate(supports):
        for variable in support:
            by_var[variable].add(ci)
    seen_support: set[frozenset[int]] = set()
    regions: list[tuple[tuple[int, ...], tuple[int, ...], tuple[int, ...]]] = []
    for ci, support in enumerate(supports):
        if not (min_width <= len(support) <= max_width) or support in seen_support:
            continue
        ...
        for variable in support:
            candidates.update(by_var[variable])
        ids = tuple(sorted(j for j in candidates if supports[j].issubset(support)))
        if len(ids) < 2:
            continue
        ordered = tuple(sorted(support))
        states = _v187_relation_states(cnf, ids, ordered)
        weights = tuple(sorted({int(x).bit_count() for x in states}))
        # Store only genuinely compressed local relations.  The truth-table
        # criterion is representation-independent: fewer than half the cube.
        if len(states) * 2 < (1 << len(ordered)):
            regions.append((ordered, ids, weights))
    return tuple(regions)


def _v187_strong_overlap_components(regions) -> tuple[int, ...]:
    if not regions:
        return tuple()
    supports = [set(region[0]) for region in regions]
...
def _v187_universal_report(cnf: CNF) -> tuple[V187UniversalReport, tuple[V187RelationAtom, ...]]:
    atoms = _v187_same_support_atoms(cnf)
    rows, _rhs = _v187_affine_shadow_equations(atoms)
    rank = _v187_gf2_rank_rows(rows)
    hamming_atoms = sum(1 for atom in atoms if len(atom.hamming_weights) == 1)
    wide = _v187_wide_hamming_closures(cnf)
    hamming_wide = tuple(region for region in wide if len(region[2]) == 1)
    overlap = _v187_strong_overlap_components(hamming_wide)
    low = _v187_low_arity_components(cnf)
    notes: list[str] = []
    if rank:
        notes.append("affine shadows derived from exact local state sets")
    if hamming_wide:
        notes.append("compressed support-closure relations with fixed Hamming layer")
    if overlap:
        notes.append("multi-variable overlap components available for macro-contraction")
    if low:
        notes.append("small low-arity interaction components admit explicit exact state spaces")
    report = V187UniversalReport(
        repeated_support_atoms=len(atoms),
        exact_affine_atoms=sum(atom.exact_affine_hyperplane is not None for atom in atoms),
        unique_affine_shadow_atoms=sum(atom.unique_max_affine_hyperplane is not None for atom in atoms),
        affine_shadow_rank=rank,
        affine_shadow_free=max(0, cnf.nvars - rank),
        hamming_layer_atoms=hamming_atoms + len(hamming_wide),
        strong_overlap_components=overlap,
        low_arity_components=low,
        wide_support_closures=len(wide),
        notes=tuple(notes),
    )
    return report, atoms


def _v187_print_report(cnf: CNF, report: V187UniversalReport) -> None:
    print("UNIVERSAL RELATION CALCULUS AUDIT")
    print(f"  variables/clauses : {cnf.nvars}/{len(cnf.clauses)}")
    print(f"  repeated-support atoms : {report.repeated_support_atoms}")
    print(f"  exact affine atoms     : {report.exact_affine_atoms}")
    print(f"  unique affine shadows  : {report.unique_affine_shadow_atoms}")
    print(f"  affine shadow rank/free: {report.affine_shadow_rank}/{report.affine_shadow_free}")
    print(f"  Hamming-layer atoms    : {report.hamming_layer_atoms}")
    print(f"  wide support closures  : {report.wide_support_closures}")
    if report.strong_overlap_components:
        from collections import Counter as _Counter
        print(f"  strong-overlap comps   : {dict(sorted(_Counter(report.strong_overlap_components).items()))}")
    if report.low_arity_components:
        from collections import Counter as _Counter
        print(f"  low-arity component signatures: {dict(sorted(_Counter(report.low_arity_components).items()))}")
    print("  benchmark/family       : NOT READ")
    print("  residual feedback      : NONE")
    print("  status                 : experimental compiler audit; certified v186 fallback retained")


@dataclass(frozen=True)
class V187AffineShadowChart:
    rows: tuple[int, ...]
    rhs: tuple[int, ...]
    rank: int
...
def _v187_try_affine_shadow_chart(cnf: CNF, atoms: tuple[V187RelationAtom, ...], max_free: int = 12) -> V187AffineShadowChart | None:
    rows0, rhs0 = _v187_affine_shadow_equations(atoms)
    if not rows0:
        return None
    # Full GF(2) elimination with RHS.  A unique local affine shadow may be a
    # selector rather than an equivalence, so success is certified later only
    # by exact symbolic intersection with every original clause.
    basis: dict[int, tuple[int, int]] = {}
    for mask0, bit0 in zip(rows0, rhs0):
        mask = int(mask0)
        bit = int(bit0)
        while mask:
        ...
        if mask == 0 and bit:
            return None
    rank = len(basis)
    free = tuple(v for v in range(cnf.nvars) if v not in basis)
    if len(free) > max_free:
        return None
    rows = tuple(basis[p][0] for p in sorted(basis))
    rhs = tuple(basis[p][1] for p in sorted(basis))
    return V187AffineShadowChart(rows=rows, rhs=rhs, rank=rank, free_variables=free, source_atoms=len(rows0))


def _v187_affine_assignment(cnf: CNF, chart: V187AffineShadowChart, free_word: int) -> np.ndarray:
    # Reconstruct from a fresh pivot-indexed basis to avoid relying on row order.
    basis: dict[int, tuple[int, int]] = {}
    for mask, bit in zip(chart.rows, chart.rhs):
        basis[int(mask).bit_length() - 1] = (int(mask), int(bit))
    bits = 0
    for j, variable in enumerate(chart.free_variables):
        if (free_word >> j) & 1:
            bits |= 1 << variable
    for pivot in sorted(basis):
        mask, bit = basis[pivot]
        rest = mask & ~(1 << pivot)
        value = bit ^ ((rest & bits).bit_count() & 1)
        if value:
            bits |= 1 << pivot
    return np.asarray([(bits >> i) & 1 for i in range(cnf.nvars)], dtype=np.uint8)


def _v187_try_affine_shadow_section(cnf: CNF, chart: V187AffineShadowChart) -> tuple[np.ndarray | None, dict]:
    tested = 0
    for free_word in range(1 << len(chart.free_variables)):
        assignment = _v187_affine_assignment(cnf, chart, free_word)
        tested += 1
        unsat, _ = verify_assignment_independent(cnf, assignment)
        if int(unsat) == 0:
            return assignment, {
                "source_atoms": int(chart.source_atoms),
                "rank": int(chart.rank),
                "free_dimension": int(len(chart.free_variables)),
                "symbolic_sections_tested": int(tested),
                "selector_kind": "unique_maximal_affine_subrelation",
            }
    return None, {
        "source_atoms": int(chart.source_atoms),
        "rank": int(chart.rank),
...
def _v187_main_affine_shadow(args, cnf: CNF, chart: V187AffineShadowChart, assignment: np.ndarray, meta: dict, total_started: float) -> int:
    print(f"=== DREAM6 {VERSION} ===")
    print("GENERIC AFFINE-SHADOW CERTIFIED SECTION")
    print(f"  variables/clauses : {cnf.nvars}/{len(cnf.clauses)}")
    print(f"  relation atoms    : {chart.source_atoms}")
    print(f"  GF(2) rank/free   : {chart.rank}/{len(chart.free_variables)}")
    print("  construction      : exact local state enumeration -> unique maximal affine shadows -> global rank closure")
    print("  benchmark/family  : NOT READ")
    print("  residual feedback : NONE")
    return _v186_emit_section(
        args, cnf, assignment, total_started,
        chart_kind="v187_generic_affine_shadow_section",
        certificate="unique maximal affine subrelations derived from arbitrary repeated-support relations; exact symbolic intersection with the untouched original CNF",
        details=meta,
    )
...
def main_v187() -> int:
    ...
    universal_report, universal_atoms = _v187_universal_report(cnf)
    _v187_print_report(cnf, universal_report)

    # Frozen v185 constructive trace remains first because it is already a
    # complete proof-carrying section when it fires.
    trace_chart = _v185_try_ordered_frequency_trace_chart(cnf)
    if trace_chart is not None:
        return _v185_main_ordered_frequency_trace(args, cnf, trace_chart, total_started)

    # First genuine v187 replacement of a named chart: derive affine shadows
    # only from exact local state sets.  No XOR/parity syntax is inspected.
    affine_shadow = _v187_try_affine_shadow_chart(cnf, universal_atoms)
    if affine_shadow is not None:
        assignment, shadow_meta = _v187_try_affine_shadow_section(cnf, affine_shadow)
        if assignment is not None:
            return _v187_main_affine_shadow(args, cnf, affine_shadow, assignment, shadow_meta, total_started)
